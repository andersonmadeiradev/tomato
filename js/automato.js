    ...
    if (e.which == 1) {
      ...
      while (k >= 0) {
        ...
        // ou seja, se o novo circulo naquela posição não colidir com outro.
          ...
          //if(!(new_node.collide(Graph.node_list))) {
            ...
            Graph.node_list.push(new Node(App.mouse.x, App.mouse.y, App.RADIUS, App.LINEWIDTH));  
      ...
      // se não, se estiver no modo de transição
       ...
       /* if (k >= 0) {
          ...
          angle = Math.atan2(mouseY - Graph.node_list[k].y, mouseX - Graph.node_list[k].x);
    ...
    if (e.which == 1) {
      ...
      // se está no modo edição de transições e se uma origem para a transição foi selecionada.
        ...
        if (App.trans_state_selected != -1) {
          ...
          k = Graph.node_list.length - 1;
          while (k >= 0) {
            if (Graph.node_list[k].collidePoint(mouseX, mouseY))
              break;
            k--;
          }
...
function drawApp() {
  ...
  tmp_canvas.width = App.canvas.width;
  tmp_canvas.height = App.canvas.height;
  
  tmp_ctx.save();
  // clear the screen
  tmp_ctx.fillStyle = 'white';
  tmp_ctx.fillRect(0, 0, tmp_ctx.width, tmp_ctx.height);
  // draw the Graph.node_list
  k = Graph.node_list.length - 1;
  while (k >= 0) {
    Graph.node_list[k].draw(tmp_ctx);
    k--;
  }
  tmp_ctx.restore();
  ...
  App.ctx.drawImage(tmp_canvas, 0, 0);
}
/*
function drawApp() {
  App.ctx.save();
